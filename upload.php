  ...
  $extension = end($tmp);
   ...
   $temp_file= $_FILES['fileToUpload']['tmp_name'];
    $thumb=  store_uploaded_image($fileToUpload,$temp_file, 200, 100);
            $uploadOk = 0;
               $message=$newobj->insertimage("uploads/$fileToUpload",$albumID,$caption,$thumb);
              header("location: add_photos.php?success=$message"); // redirects the user to the add photos page
          exit;
               }
               else{
                     $temp_file= $_FILES['fileToUpload']['tmp_name'];
                     $thumb= store_uploaded_image($fileToUpload, $temp_file,200, 100);
                      $message=$newobj->insertimage($fullimage_file,$albumID,$caption,$thumb);
                      header("location: add_photos.php?success=$message");
                      exit;
                        }
         }
     else {
